...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingUsers = await sql`SELECT id, email FROM users WHERE email = ${email} LIMIT 1`

    if (existingUsers.length > 0) {
      // Update existing user with new temporary password
      await sql`
        UPDATE users
        SET password_hash = ${passwordHash}, name = ${name}, updated_at = now()
        WHERE email = ${email}
      `
      ...
      await sql`
        ...
        VALUES (${email}, ${passwordHash}, ${name}, 'investor')
      `
