...
export async function fetchBottomUpMetrics(): Promise<BottomUpMetrics> {
  ...
  // Fetch metrics from the Bottom up! tab
  const metrics: BottomUpMetrics = {
    avgCheckSize: await getStringValue(sheetId, "'Bottom up'!B2"),
    targetPortfolio: await getStringValue(sheetId, "'Bottom up'!B3"),
    failureRate: await getStringValue(sheetId, "'Bottom up'!B4"),
    profitSharingStartYear: await getStringValue(sheetId, "'Bottom up'!B5"),
    netIncomeMultiple: await getStringValue(sheetId, "'Bottom up'!B6"),
    revenueMultiple: await getStringValue(sheetId, "'Bottom up'!B7"),
    yearlyData: []
  };
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const yearData = {
      year: `Year ${i + 1}`,
      annualProfitSharing: await getNumberValue(sheetId, `'Bottom up'!K${rowIndex}`),
      cumulativeDistributions: await getNumberValue(sheetId, `'Bottom up'!L${rowIndex}`),
      grossTVPI: await getNumberValue(sheetId, `'Bottom up'!S${rowIndex}`),
      netTVPI: await getNumberValue(sheetId, `'Bottom up'!T${rowIndex}`)
    };
